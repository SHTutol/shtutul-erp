...
import { DebitVoucherData, UnitRecord, PayeeRecord, SisterRecord, ViewType } from '../types';
...
export const DebitVoucher: React.FC<DebitVoucherProps> = ({ 
  ...
  const handlePayeeSelect = (payeeName: string) => {
    ...
    setData(prev => ({ 
      ...prev, 
      paidTo: payeeName,
      for: payeeRecord?.particulars ? payeeRecord.particulars : prev.for
    }));
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>Print Debit Voucher - ${data.no}</title>
          ${styles}
          <style>
            @page { size: 7in 8in; margin: 0.2in; }
        ...
        <div className="no-print mb-4 w-full max-w-[7in] flex justify-between items-center bg-white p-4 rounded-3xl border border-slate-200 shadow-xl">
           ...
           <div className="flex gap-2">
             ...
             <button onClick={handlePrint} className="flex items-center gap-2 px-8 py-3 bg-slate-900 text-white font-bold rounded-2xl text-xs hover:bg-slate-800 transition-all shadow-2xl active:scale-95 uppercase">
               <Printer size={16} /> Print Debit Voucher
             </button>
           </div>
        </div>
