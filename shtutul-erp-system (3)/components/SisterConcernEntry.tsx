...
import { SisterRecord, PayeeRecord } from '../types';
import { 
  Plus, 
  Pencil, 
  FileText, 
...
interface SisterConcernEntryProps {
  onViewChange: (view: ViewType) => void;
  sisters: SisterRecord[];
  onUpdateSisters: (sisters: SisterRecord[]) => void;
  availablePayees: PayeeRecord[];
}

export const SisterConcernEntry: React.FC<SisterConcernEntryProps> = ({ 
  onViewChange, 
  sisters, 
  onUpdateSisters,
  availablePayees = []
  ...
  const handleModalSave = () => {
    if (!newConcernName.trim()) {
      alert("Please enter a concern name.");
      return;
    }
    
    const nextSl = sisters.length > 0 ? Math.max(...sisters.map(s => s.sl)) + 1 : 1;
    const newId = Math.random().toString(36).substr(2, 9);
    
    const newRecord: SisterRecord = {
      id: newId,
      sl: nextSl,
      name: newConcernName.trim(),
      phone: '' 
    };
    
    onUpdateSisters([...sisters, newRecord]);
    setSelectedId(newId);
  ...
  const handleNameChange = (id: string, value: string) => {
    onUpdateSisters(sisters.map(s => s.id === id ? { ...s, name: value } : s));
  };

  const handlePhoneChange = (id: string, value: string) => {
    onUpdateSisters(sisters.map(s => s.id === id ? { ...s, phone: value } : s));
  };

  const handleDeleteRow = () => {
    if (selectedId && window.confirm("Are you sure you want to delete this Sister Concern?")) {
      const updated = sisters.filter(s => s.id !== selectedId);
      const reindexed = updated.map((item, i) => ({ ...item, sl: i + 1 }));
      onUpdateSisters(reindexed);
      setSelectedId(null);
    } else if (!selectedId) {
  ...
  return (
    <div className="w-full max-w-[1200px] flex flex-col bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden relative">
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-[#EFEFEF] border-b border-gray-300 text-[#333] font-bold text-[12px] uppercase tracking-wide">
              ...
              <th className="px-4 py-2 border-r border-gray-300">Sister Concern Name</th>
      ...
      <div className="bg-[#F3F3F3] border-t border-gray-300 p-2 flex justify-between items-center text-[11px] text-gray-500 font-bold uppercase">
        <span>Total Concerns: {filtered.length}</span>
        <span>Selected: {selectedId ? sisters.find(s => s.id === selectedId)?.name : 'None'}</span>
      </div>

      {/* Add Sister Concern Modal */}
      {isModalOpen && (
            ...
            <div className="bg-slate-900 px-8 py-6 flex justify-between items-center">
              <div>
                <h3 className="text-white font-black uppercase tracking-widest text-sm flex items-center gap-2">
                  <PlusCircle size={18} className="text-blue-400" /> Add Sister Concern
                </h3>
            ...
            <div className="p-10">
              <div className="space-y-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Concern Name</label>
                  <div className="relative">
                    <Building className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" size={18} />
                    <input 
                      ref={modalInputRef}
                      type="text" 
                      placeholder="E.G. AZLAN AGRO LTD."
                      value={newConcernName}
                      onChange={(e) => setNewConcernName(e.target.value)}
                      onKeyDown={(e) => e.key === 'Enter' && handleModalSave()}
                      className="w-full pl-12 pr-4 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl font-black text-slate-900 outline-none focus:border-blue-600 focus:bg-white transition-all uppercase placeholder:text-slate-300 text-lg shadow-inner"
                    />
                  </div>
                </div>
              </div>
              <div className="mt-10 flex gap-4">
                ...
                <button 
                  ...
                  className="flex-2 px-8 py-4 rounded-2xl bg-blue-600 font-black text-white hover:bg-blue-700 transition-all shadow-xl shadow-blue-500/20 text-xs uppercase flex items-center justify-center gap-2 active:scale-95"
                >
